...
function App() {
	...
	const { data, error, loading } = useFetch<User>('/1/profile', 'GET')
  ...
  if (error) return <div>Error: {error}</div>;
	...
	return (
				...
				<div className="container">
					<div className="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
						...
						<ul className='nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0'>
							...
							<li><a href="#" className="nav-link px-2 text-white">{data?.username}</a></li>
		...
		<div className="container px-4 py-5 text-center">
			<img src="" alt="" />
			<h1 className='text-5xl fw-bold text-body-emphasis pb-4'>{data?.blog_title}</h1>
			<div className="col-lg-6 mx-auto">
			<p className="lead">{data?.blog_subtitle}</p>
		...
		<main className='container py-5'>
			...
			<div className='grid sm:grid-cols-1 xl:grid-cols-2 gap-3'>
				{notifications.map((notification) => (
					<BlogCard key={notification.id} title={notification.title} subtitle={notification.subtitle} datetimePublished={notification.datetimePublished} />
				))}
